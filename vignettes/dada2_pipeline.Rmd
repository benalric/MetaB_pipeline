...
ASV_func <- function(annot, ASVt, id, vtot, vocc) {
  ...
  occurrence <- apply(ASVt[, grep(id, colnames(ASVt))], 1, function(x) sum(x > 0))
  ...
  annot1 <- rbindlist(annot) %>%
    ...
    rename(identity = confidence, taxonomy = taxon) %>% 
  ...
  ASV_table <- merge(annot1, ASVt, by.x = "amplicon", by.y = "amplicon", all.x = TRUE) %>%
    ...
    relocate(amplicon, taxonomy, rank, identity, sequence, total, occurrence, PHP_B8_190328_02_PHYTOPORT_Cut1:PHP_H42_200304_3_PHYTOPORT_Cut2)
  ...
  ASV_table_filtered <- ASV_table[total >= vtot & occurrence >= vocc][order(total, decreasing = TRUE)]
